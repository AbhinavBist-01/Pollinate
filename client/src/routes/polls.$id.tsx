...
function PollDetail() {
  ...
  async function togglePublish() {
    ...
    setPoll((prev: any) => prev ? { ...prev, status: nextLive ? 'live' : 'draft', isPublished: nextLive } : prev)
    try {
      await api.patch(`/api/polls/${id}`, { status: nextLive ? 'live' : 'draft', isPublished: nextLive })
    } catch {
      setPoll((prev: any) => prev ? { ...prev, status: poll?.status, isPublished: poll?.isPublished } : prev)
    }
  }

  function copyLink() { navigator.clipboard.writeText(`${window.location.origin}/p/${poll.shareId}`) }
  function copyJoinCode() { navigator.clipboard.writeText(poll.shareId) }
  async function endPoll() {
    await api.patch(`/api/polls/${id}`, { status: 'ended', isPublished: false })
    setPoll((prev: any) => prev ? { ...prev, status: 'ended', isPublished: false, endedAt: new Date().toISOString() } : prev)
  }
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex items-center gap-2">
          {(poll.status === 'live' || poll.isPublished) && <Button variant="outline" onClick={copyLink}>Copy Link</Button>}
          <Button asChild variant="secondary">
            <Link to="/polls/$id/results" params={{ id }}>View Live Results</Link>
          </Button>
          {poll.status === 'live' && <Button onClick={endPoll} variant="destructive">End Poll</Button>}
          <Button onClick={togglePublish} variant={poll.status === 'live' ? 'outline' : 'default'} disabled={poll.status === 'ended'}>
          ...
          <CardContent className="grid gap-4 p-4 text-sm md:grid-cols-[1fr_auto] md:items-center">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-md border border-white/10 bg-secondary px-2 py-1 font-mono text-foreground">{poll.shareId}</span>
                <Button variant="ghost" size="sm" onClick={copyJoinCode}>Copy code</Button>
              </div>
            </div>
            <img
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <Card className="border-white/10 bg-card/90 transition-colors hover:border-primary/35">
          <Link to="/polls/$id/results" params={{ id }} className="block p-5 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Watch responses update</p>
          </Link>
        </Card>
        <Card className="border-white/10 bg-card/90 transition-colors hover:border-primary/35">
