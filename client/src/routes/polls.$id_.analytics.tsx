...
function Analytics() {
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-white">{data.title || 'Analytics'}</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-5 text-center">
          <p className="text-3xl font-bold text-orange">{data.completionRate ?? 100}%</p>
          <p className="text-xs text-white/40 mt-1">Completion Rate</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/5 p-5 text-center">
          <p className="text-3xl font-bold text-white">{data.responsesOverTime?.length ?? 0}</p>
              ...
              return (
                <div key={r.date}>
                  ...
                  <div className="h-3 rounded-full bg-white/10 overflow-hidden">
                    <div className="h-full rounded-full bg-orange transition-all" style={{ width: `${(r.count / maxCount) * 100}%` }} />
                  </div>
                </div>
              )
            })}
